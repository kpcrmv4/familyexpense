...
import { formatThaiDate, todayDateString } from '../utils/formatters';
...
export async function showSummary(
  ...
  const [summary, categoryBreakdown, recurringItems, accumulatedBalance, todaySummary, recentTransactions] = await Promise.all([
    ...
    transactionService.getTodaySummary(user.id),
    transactionService.getRecentTransactions(user.id, 5),
  ]);

  const todayDisplay = formatThaiDate(todayDateString());

  await lineClient.replyMessage({
    replyToken,
    messages: [summaryFlex.monthlySummaryMessage(summary, categoryBreakdown, recurringItems, theme, targetYear, targetMonth, accumulatedBalance, todaySummary, recentTransactions, todayDisplay)],
  });
}
