...
function buildPaymentFooterButtons(
  ...
  if (status.isPaid) {
    ...
    buttons.push(
      createButton('✅ จ่ายทันที', JSON.stringify({ action: 'reminder_paid', id: item.id }), 'primary', '#68D391')
    );
      ...
      contents: [
        {
          ...
          layout: 'horizontal',
          contents: [
            { type: 'text', text: '💳 ยอดหนี้', size: 'xxs', color: '#718096', flex: 1 },
            { type: 'text', text: `฿${formatCurrency(totalDebt)}`, size: 'xxs', color: '#2D3748', align: 'end', weight: 'bold' },
          ],
        },
        createSpacer('xs'),
        // Progress bar
        ...
        createSpacer('xs'),
        {
          ...
          layout: 'horizontal',
          contents: [
            { type: 'text', text: `จ่ายแล้ว ฿${formatCurrency(totalPaid)}`, size: 'xxs', color: '#68D391', flex: 1 },
            { type: 'text', text: `คงเหลือ ฿${formatCurrency(remaining)}`, size: 'xxs', color: '#FC8181', align: 'end' },
          ],
        },
        {
          type: 'text',
          ...
          size: 'xs',
          weight: 'bold',
          color: paidPercent >= 100 ? '#68D391' : '#F6C065',
          align: 'center',
        },
      ],
                ...
                },
                {
                  ...
                  contents: [
                    ...(paidPercent > 0 ? [{
                      ...
                      backgroundColor: '#68D391', height: '6px', flex: paidPercent || 1,
                    }] : []),
                    ...(100 - paidPercent > 0 ? [{
                      type: 'box' as const, layout: 'vertical' as const, contents: [] as any[],
                      backgroundColor: '#E8ECF0', height: '6px', flex: (100 - paidPercent) || 1,
                    }] : []),
                  ],
                  cornerRadius: 'md',
                  margin: 'xs',
...
export function recurringEditMessage(item: RecurringExpense, theme: ThemeColors): any {
  ...
  const footerButtons = [
    ...
    createButton('🗑️ ลบรายการนี้', JSON.stringify({ action: 'recurring_delete', id: item.id }), 'primary', '#FC8181'),
...
export function debtEditMessage(item: RecurringExpense, theme: ThemeColors): any {
  ...
  const footerButtons = [
    ...buildPaymentFooterButtons(item, status),
    createButton('✏️ แก้ไขยอดหนี้รวม', JSON.stringify({ action: 'debt_update_balance', id: item.id }), 'primary', '#F6C065'),
    createButton('➕ รูดบัตรเพิ่ม (เพิ่มยอดหนี้)', JSON.stringify({ action: 'debt_add_charge', id: item.id }), 'primary', '#FC8181'),
  ...
  return createFlexMessage('ยืนยันลบ', createBubble({
    ...
    footer: {
      ...
      contents: [
        ...
        createButton('🗑️ ลบเลย', JSON.stringify({ action: 'recurring_confirm_delete', id: item.id }), 'primary', '#FC8181'),
      ],
      paddingAll: 'lg',
      spacing: 'sm',
    },
